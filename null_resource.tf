resource "null_resource" "commands_to_run" {
  provisioner "remote-exec" {
    connection {
        ...
        host            = "${aws_instance.centos.public_ip}"  #to connect the remoote machine to run command
    }
    inline = [
...
#         type            = "ssh"
#         user            = "centos"
#         private_key     = "${file("~/.ssh/id_rsa")}"                      2ND WAY BOOTSTRAPPING
...
#     connection {
#         type            = "ssh"
#         user            = "centos"                                        3RD WAY BOOTSTRAPPING
#         private_key     = "${file("~/.ssh/id_rsa")}"
#         host            = "${aws_instance.centos.public_ip}"
#     }
